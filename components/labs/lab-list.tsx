        ...
        {labs.map((lab) => (
          <div
            ...
            className={`
              ...
              ${lab.is_featured
                ? "bg-gradient-to-br from-white/90 to-emerald-50/40 border-2 border-emerald-300/70 hover:border-emerald-400 hover:shadow-md hover:shadow-emerald-100/50"
                : "bg-white/80 border-2 border-gray-200/80 hover:border-gray-300 hover:shadow-md hover:shadow-gray-200/50"
              }
            `}
          >
            {/* おすすめバッジ */}
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              {lab.rating && (
                <div className="flex items-center gap-0.5 flex-shrink-0 bg-gradient-to-br from-yellow-50 to-amber-50 px-1.5 py-0.5 rounded-lg border border-yellow-200/60">
            ...
            <div className="grid grid-cols-2 gap-1.5 mt-2 pl-9">
              <div className="flex items-center gap-1 bg-gray-50/80 rounded-lg px-2 py-1 border border-gray-200/50">
                <Clock className="w-3 h-3 text-blue-500 flex-shrink-0" />
                <span className="text-[10px] text-gray-400">納期</span>
                <span className="text-[11px] text-gray-700 font-semibold ml-auto">{formatDelivery(lab.delivery_days_min, lab.delivery_days_max)}</span>
              </div>
              <div className="flex items-center gap-1 bg-gray-50/80 rounded-lg px-2 py-1 border border-gray-200/50">
                <Banknote className="w-3 h-3 text-emerald-500 flex-shrink-0" />
                <span className="text-[10px] text-gray-400">料金</span>
                <span className="text-[11px] text-gray-700 font-semibold ml-auto">{formatPrice(lab.price_min, lab.price_max)}</span>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-1 mt-1.5 pl-9">
              {lab.service_area && (
                <span className="text-[10px] px-1.5 py-px rounded-md bg-blue-50/80 text-blue-600 border border-blue-200/50 font-medium">
                  {lab.service_area}
                </span>
              )}
              {lab.track_record && (
                <span className="text-[10px] px-1.5 py-px rounded-md bg-purple-50/80 text-purple-600 border border-purple-200/50 font-medium">
                  実績 {lab.track_record.toLocaleString()}件
                </span>
              )}
              {lab.certifications && lab.certifications.map((cert) => (
                <span key={cert} className="inline-flex items-center gap-0.5 text-[10px] px-1.5 py-px rounded-md bg-emerald-50/80 text-emerald-700 border border-emerald-200/50 font-medium">
            ...
            {/* 5行目: 連絡先 */}
            <div className="flex items-center gap-2 mt-2 pl-9 pt-2 border-t border-gray-200/50">
              {lab.phone && (
                <a
                  href={`tel:${lab.phone}`}
                  className="inline-flex items-center gap-1 text-xs font-medium text-emerald-600 hover:text-emerald-700 bg-emerald-50/50 hover:bg-emerald-50 px-2 py-1 rounded-lg border border-emerald-200/50 transition-all"
                >
              ...
              {lab.website_url && (
                <a
                  ...
                  className="inline-flex items-center gap-1 text-xs font-medium text-gray-600 hover:text-gray-800 bg-gray-50/50 hover:bg-gray-100/80 px-2 py-1 rounded-lg border border-gray-200/50 transition-all"
                >
